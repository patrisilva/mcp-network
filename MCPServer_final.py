...
if not USERNAME or (not PASSWORD and not SSH_KEY_PATH):
    raise RuntimeError("Credentials not set - need USERNAME and either PASSWORD or SSH_KEY_PATH")
...
async def run_show(params: ShowCommand) -> str:
    ...
    if SSH_KEY_PATH:
        connection["auth_private_key"] = SSH_KEY_PATH
    else:
        connection["auth_password"] = PASSWORD
